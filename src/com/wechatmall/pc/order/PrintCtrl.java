...
public class PrintCtrl extends BaseCtrl {
    ...
    public void printSingleOrder()throws UnsupportedEncodingException{
        ...
        JSONArray orderIdList=JSONArray.fromObject(idList);
        int idLen=orderIdList.size();
        String content="";
        PDFUtil pdfUtil = new PDFUtil(this.getRequest(), "template");

        //遍历订单id
        for (int i=0;i<idLen;i++){
            String orderId=orderIdList.getJSONObject(i).getString("id");
...
//        }
            ...
            if(dataList != null && dataList.size() > 0){
                int k = 1;
                for(int j = 0; j < dataList.size(); j++,k++){
            ...
            content += pdfUtil.loadDataByTemplate(data, "orderTemplate.html");
        }
            try {
...
//            this.getRequest().getRequestDispatcher(getRequest().getContextPath()  + "/pdf/a.pdf");
                ...
                result.set("pdfUrl", getRequest().getScheme()+"://"+getRequest().getServerName()+":"+getRequest() .getServerPort()  + "/pdf/" + name + ".pdf");
